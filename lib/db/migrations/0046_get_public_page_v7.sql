-- 0046_get_public_page_v7.sql — 공개 학생 안내 페이지 v7 (보안점검 2026-07 ③).
--
-- v6(0043) 기반 재정의. 함수명·시그니처 동일(create or replace) → 호출부 변경 불필요.
--
-- v7 변경점 (딱 한 곳 추가):
--   (a) weekTodos 의 calendar_events(manual/neis) 분기에 `ce.is_public = true` 필터 추가.
--       교사가 학생 비공개로 표시한 이벤트(0045 is_public=false)는 학생 페이지에
--       노출하지 않는다. 상담예약 분기·다른 섹션은 절대 변경하지 않는다(회귀 방지).
--   ⚠ v6 의 다른 섹션(studentMemos, 급식 KST, 출결 1D/2D 등)은 그대로 유지.
--
-- 0008 의 token→not_found/revoked/expired 판정 로직을 유지한다.
-- 함수는 create or replace 라 재적용 안전(기존 ACL 보존 — 0047 revoke 이후에도 유지).
-- ⚠ 커스텀 SQL(드리즐 저널 외) — DB 리셋 시 0045 다음에 따로 적용(오케스트레이터).
...
comment on function get_public_page(text) is
  '공개 학생 안내 페이지 유일 읽기 진입점(v7, 0046). v6(0043) 기반. 변경점: (a) weekTodos 의 calendar_events 분기에 is_public=true 필터(학생 비공개 이벤트 제외, 0045). 나머지 동작은 v6 동일: 토큰→단일 student_year_id, studentName, 전체/개별 공지, 월간 일정창∪상담예약, 시간표, 당일 급식(KST), 출결 1D/2D, 상담 슬롯, studentMemos, 성적=preparing. 전 필드 NULL-safe.';
