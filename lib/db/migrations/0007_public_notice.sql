-- 0007: 공지실(Phase2-I) — 공개 페이지 공통 '교사 한마디' 소스 연결
--
-- (1) teacher_profile 에 public_notice 컬럼 추가(공통 공지/교사 한마디).
-- (2) get_public_page 의 commonNotice 를 teacher_profile.public_notice 로 연결
--     (0001 에서 하드코딩 null 이던 자리). 함수는 create or replace 라 재적용 안전.
--
-- ⚠ 이 파일은 커스텀 SQL(드리즐 저널 외) — DB 리셋 시 0001~0006 다음에 따로 적용.

alter table teacher_profile add column if not exists public_notice text;
...
comment on function get_public_page(text) is
  '공개 학생 페이지 유일 읽기 진입점. 토큰→단일 student_year_id, 출결 횟수 사전집계, 성적 목업 준비중, 공통 한마디=teacher_profile.public_notice. reason/note_field/원점수 미반환 (계획 §3.2, 0007).';
