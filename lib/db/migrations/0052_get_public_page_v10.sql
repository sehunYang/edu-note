-- 0052_get_public_page_v10.sql — 공개 학생 안내 페이지 v10 (공지 입력일 노출).
--
-- v9(0051) 기반 재정의. 함수명·시그니처 동일(create or replace) → 호출부 변경 불필요.
--
-- v10 변경점 (딱 두 곳):
--   (a) notices 를 문자열 배열이 아니라 { body, createdAt } 객체 배열로 반환.
--       createdAt = teacher_notes.created_at(교사 입력 시각, timestamptz → ISO).
--       학생 페이지가 "언제 올라온 공지인지" 표시 + 최근(2일 이내) New 배지를 그릴 수 있게 한다.
--   (b) individualNotices 도 동일하게 { body, createdAt } 객체 배열로 변경.
--   ⚠ commonNotice(단일, 하위호환)는 여전히 body 문자열 그대로 유지.
--   ⚠ v9 의 다른 섹션(weekTodos eventKind, vacationSpans, attendanceDetail, studentMemos,
...
-- ⚠ 커스텀 SQL(드리즐 저널 외) — DB 리셋 시 0048~0051 다음에 따로 적용(오케스트레이터).
...
begin
  ...
  v_payload := jsonb_build_object(
    ...
    -- v10: notices = { body, createdAt } 객체 배열(입력일 표시·New 배지용). createdAt=교사 입력 시각.
    'notices', coalesce(
      (select jsonb_agg(
                jsonb_build_object('body', tn.body, 'createdAt', tn.created_at)
                order by tn.sort_order, tn.created_at)
       from teacher_notes tn
       where tn.owner_id = v_owner
         and coalesce(tn.target_scope, 'all') = 'all'), '[]'::jsonb),
    -- 개별 공지(AC-5.3): 이 학생 대상으로 매핑된 'individual' 한마디만. 'all' 과 병렬.
    -- v10: 동일하게 { body, createdAt } 객체 배열.
...
comment on function get_public_page(text) is
  '공개 학생 안내 페이지 유일 읽기 진입점(v10, 0052). v9(0051) 기반. 변경점: notices·individualNotices 를 { body, createdAt } 객체 배열로 반환(createdAt=teacher_notes.created_at, 학생페이지 공지 입력일 표시·최근 2일 New 배지용). commonNotice 는 여전히 단일 body 문자열. 나머지 동작은 v9 동일: weekTodos(eventKind)·vacationSpans·시간표·당일 급식(KST)·출결 1D/2D/상세·상담 슬롯·studentMemos·성적=preparing. 전 필드 NULL-safe.';
