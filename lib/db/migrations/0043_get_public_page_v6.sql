-- 0043_get_public_page_v6.sql — 공개 학생 안내 페이지 v6 (QC v6 ⑤, AC-5.3/5.4).
--
-- v5(0040) 기반 재정의. 함수명·시그니처 동일(create or replace) → 호출부 변경 불필요.
--
-- v6 변경점 (딱 한 곳 추가):
--   (a) AC-5.4 학생 개인 메모/일정 노출: payload 에 `studentMemos` 섹션 신규 추가.
--       student_calendar_memos 를 **v_sy_id 스코프**로만 조회(본인 메모만, 타학생·교사 비노출).
--       각 항목 { id, date, body } — 학생 캘린더 모달의 조회/수정/삭제(CRUD)용. id 는 본인
--       메모 식별자라 노출 OK(내부 owner/student id 등 민감키는 미포함). dto allowlist 에
--       studentMemos 키를 추가해야 파서가 보존한다(parsePublicPagePayload).
--   ⚠ v5 의 다른 섹션(급식 KST, current_date 섹션 등)은 절대 변경하지 않는다(회귀 방지).
--
-- 0008 의 token→not_found/revoked/expired 판정 로직을 유지한다.
-- 함수는 create or replace 라 재적용 안전. 0040/0042 적용 후에 적용한다.
-- ⚠ 커스텀 SQL(드리즐 저널 외) — DB 리셋 시 0040 다음에 따로 적용(오케스트레이터).
...
comment on function get_public_page(text) is
  '공개 학생 안내 페이지 유일 읽기 진입점(v6, 0043). v5(0040) 기반. 변경점: (a) studentMemos 섹션 추가 — student_calendar_memos 를 v_sy_id 스코프로만 노출({id,date,body}, 학생 캘린더 모달 CRUD용). 나머지 동작은 v5 동일: 토큰→단일 student_year_id, studentName, 전체/개별 공지, 월간 일정창∪상담예약, 시간표, 당일 급식(KST), 출결 1D/2D, 상담 슬롯, 성적=preparing. 전 필드 NULL-safe.';
