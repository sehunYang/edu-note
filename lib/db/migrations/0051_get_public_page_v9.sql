-- 0051_get_public_page_v9.sql — 공개 학생 안내 페이지 v9 (학생 캘린더 색상·방학 밴드).
--
-- v8(0048) 기반 재정의. 함수명·시그니처 동일(create or replace) → 호출부 변경 불필요.
--
-- v9 변경점 (딱 세 곳):
--   (a) weekTodos 의 calendar_events 분기 item 에 `'eventKind', ce.event_kind` 추가 —
--       학생페이지 캘린더가 오늘의학교와 동일한 event_kind 색상 칩을 그릴 수 있게 한다.
--   (b) weekTodos 의 상담예약 분기 item 에 `'eventKind', 'counsel'` 추가 — 오늘의학교의
--       green 상담칩과 대응하는 고정 리터럴(calendar_events 컬럼이 아니므로 하드코딩).
--   (c) payload 에 `'vacationSpans'` 추가 — academic_vacations(0050)에서 현재 조회창
--       (current_date-31 ~ current_date+93)과 겹치는 방학 구간을 [{start,end}] 배열로 노출.
--       학생페이지 달력이 방학 배경 밴드(주말 포함)를 그릴 수 있게 한다.
--   ⚠ v8 의 다른 섹션(attendanceDetail, studentMemos, 급식 KST, 출결 1D/2D 등)은
--     그대로 유지(회귀 방지).
--
-- 0008 의 token→not_found/revoked/expired 판정 로직을 유지한다.
-- 함수는 create or replace 라 재적용 안전(기존 ACL 보존 — 0047 revoke 이후에도 유지).
-- ⚠ 커스텀 SQL(드리즐 저널 외) — DB 리셋 시 0048~0050 다음에 따로 적용(오케스트레이터).
...
comment on function get_public_page(text) is
  '공개 학생 안내 페이지 유일 읽기 진입점(v9, 0051). v8(0048) 기반. 변경점: (a) weekTodos 항목에 eventKind 추가(calendar_events.event_kind, 상담예약은 리터럴 counsel) — 학생페이지 캘린더 색상 칩용. (b) payload.vacationSpans 추가 — academic_vacations(0050)에서 조회창과 겹치는 방학 구간 [{start,end}] — 방학 배경 밴드용. 나머지 동작은 v8 동일: 토큰→단일 student_year_id, studentName, 전체/개별 공지, 시간표, 당일 급식(KST), 출결 1D/2D/상세, 상담 슬롯, studentMemos, 성적=preparing. 전 필드 NULL-safe.';
