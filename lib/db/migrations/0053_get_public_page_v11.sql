-- 0053_get_public_page_v11.sql — 공개 학생 안내 페이지 v11 (공지 게시일 = 수정 시각).
--
-- v10(0052) 기반 재정의. 함수명·시그니처 동일(create or replace) → 호출부 변경 불필요.
--
-- v11 변경점 (딱 두 곳):
--   (a) notices 항목의 시각을 created_at → **updated_at** 으로 변경하고 키명도
--       'createdAt' → 'postedAt' 로 정정. 교사가 공지 내용을 수정하면 updated_at 이
--       갱신되므로, 학생 페이지의 게시일 표시가 수정일로 뜨고 New 배지(2일 이내)도 다시 뜬다.
--       ⚠ 순서 변경(reorderTeacherNote)은 updated_at 을 건드리지 않도록 쿼리를 함께 고쳤다
--         (재정렬만으로 New 가 오발하지 않게 — notice.ts).
--   (b) individualNotices 도 동일하게 updated_at / 'postedAt'.
--   ⚠ commonNotice(단일, 하위호환)는 여전히 body 문자열 그대로 유지.
--   ⚠ v10 의 다른 섹션(weekTodos eventKind, vacationSpans, attendanceDetail, studentMemos,
...
-- ⚠ 커스텀 SQL(드리즐 저널 외) — DB 리셋 시 0048~0052 다음에 따로 적용(오케스트레이터).
...
begin
  ...
  v_payload := jsonb_build_object(
    ...
    -- v11: notices = { body, postedAt } 객체 배열. postedAt=교사 수정 시각(updated_at)
    -- → 내용 수정 시 게시일 갱신·New 재노출. 순서 변경은 updated_at 미갱신(notice.ts).
    'notices', coalesce(
      (select jsonb_agg(
                jsonb_build_object('body', tn.body, 'postedAt', tn.updated_at)
                order by tn.sort_order, tn.created_at)
       from teacher_notes tn
       where tn.owner_id = v_owner
         and coalesce(tn.target_scope, 'all') = 'all'), '[]'::jsonb),
    -- 개별 공지(AC-5.3): 이 학생 대상으로 매핑된 'individual' 한마디만. 'all' 과 병렬.
    -- v11: 동일하게 { body, postedAt(updated_at) } 객체 배열.
...
comment on function get_public_page(text) is
  '공개 학생 안내 페이지 유일 읽기 진입점(v11, 0053). v10(0052) 기반. 변경점: notices·individualNotices 항목 시각을 created_at→updated_at 으로, 키명을 createdAt→postedAt 으로 정정(공지 내용 수정 시 게시일 갱신·최근 2일 New 재노출). 순서 변경(reorderTeacherNote)은 updated_at 미갱신. commonNotice 는 여전히 단일 body 문자열. 나머지 동작은 v10 동일: weekTodos(eventKind)·vacationSpans·시간표·당일 급식(KST)·출결 1D/2D/상세·상담 슬롯·studentMemos·성적=preparing. 전 필드 NULL-safe.';
