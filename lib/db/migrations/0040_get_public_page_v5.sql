-- 0040_get_public_page_v5.sql — 공개 학생 안내 페이지 v5 (QC v5 c6, AC-6.2/6.3).
--
-- v4(0036) 기반 재정의. 함수명·시그니처 동일(create or replace) → 호출부 변경 불필요.
--
-- v5 변경점 (딱 두 곳):
--   (a) AC-6.2 급식 날짜경계 KST 교정: 급식 서브쿼리의 `mc.date = current_date`(UTC) →
--       `mc.date = (now() AT TIME ZONE 'Asia/Seoul')::date`(KST). 자정~09:00 KST 지연 제거.
--       ⚠ 급식 서브쿼리에만 한정 — 함수 내 다른 섹션의 current_date(counsel slots,
--         events/상담 캘린더 등)는 절대 건드리지 않는다(S3 회귀 방지).
--   (b) AC-6.3 메뉴 항목 줄바꿈: 메뉴 합치기 separator 를 `string_agg(item, ', ')` →
--       `string_agg(item, E'\n')`(메뉴 항목 사이만 개행). mealType concat_ws 접두는 유지.
--       public-page-view.tsx 의 whitespace-pre-line 이 항목별 줄바꿈을 렌더.
--
-- 0008 의 token→not_found/revoked/expired 판정 로직을 유지한다.
-- 함수는 create or replace 라 재적용 안전. 0034/0035/0036 적용 후에 적용한다.
-- ⚠ 커스텀 SQL(드리즐 저널 외) — DB 리셋 시 0036 다음에 따로 적용(오케스트레이터).
...
comment on function get_public_page(text) is
  '공개 학생 안내 페이지 유일 읽기 진입점(v5, 0040). v4(0036) 기반. 변경점: (a) 급식 날짜경계를 KST((now() AT TIME ZONE ''Asia/Seoul'')::date)로 교정(급식 서브쿼리 한정, 다른 섹션 current_date 불변), (b) 메뉴 항목 separator 를 E''\n''(줄바꿈)로 변경(whitespace-pre-line 렌더). 나머지 동작은 v4 동일: 토큰→단일 student_year_id, studentName, 전체/개별 공지, 월간 일정창∪상담예약, 시간표, 당일 급식(KST), 출결 1D/2D, 상담 슬롯, 성적=preparing. 전 필드 NULL-safe (QC v5 c6).';
