...
describe.skipIf(!RUN)("공지실 — 공개 페이지 공통 안내", () => {
  ...
  it("get_public_page(v3 0029) 가 commonNotice(=첫 teacher_notes)·weekTodos(월간창·안전소스)를 노출", async () => {
    ...
    const rows = await sql<{ get_public_page: { state: string; payload?: { commonNotice: string | null; notices: { body: string; createdAt: string }[]; weekTodos: { title: string }[] } } }[]>`
    ...
    // v10: notices 는 { body, createdAt } 객체 배열
    expect(result.payload?.notices.map((n) => n.body)).toContain("공개용 한마디");
    expect(result.payload?.notices.every((n) => typeof n.createdAt === "string")).toBe(true);
