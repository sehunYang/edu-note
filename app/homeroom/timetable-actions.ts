...
/**
 * 담임반 시간표 컴시간 동기화 서버액션 (QC v3 Part B, US-B13, AC-12.3).
 *
 * 교사 기본설정의 컴시간 학교 + 담임 학년/반으로 학년 시간표를 파싱해
 * homeroom_timetable_slots 를 교체(replace)한다. 공개 페이지 시간표의 소스.
 * 컴시간은 비공식·변동 → 실패 시 throw 하지 않고 안내 메시지(수기 fallback) 반환.
 * (세팅실 교사 시간표 sync(syncTimetableAction)와 동일한 비차단 패턴.)
 */
...
export async function syncHomeroomTimetableAction(
  ...
  try {
    ...
    if (
      ...
      return {
        ok: false,
        message: "담임 학년/반이 설정되어 있지 않습니다. 세팅실에서 설정하세요.",
      };
    }
    ...
    if (!school) {
      return {
        ok: false,
        message:
          "컴시간 학교가 설정되어 있지 않습니다. 세팅실-수업관리에서 컴시간 동기화를 먼저 하세요.",
      };
    }
