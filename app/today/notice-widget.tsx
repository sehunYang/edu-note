...
export function NoticeWidget({
  ...
  return (
    <section className="rounded-lg border border-neutral-200 p-4 md:col-span-2">
      <h2 className="text-sm font-semibold text-neutral-700">공지</h2>
  ...
  if (notes.length === 0) {
    return (
      <div className="rounded border border-neutral-100 bg-neutral-50 p-3">
        <p className="text-xs font-semibold text-neutral-400">교사 한마디</p>
  ...
  return (
    <div className="rounded border border-neutral-100 bg-neutral-50 p-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-neutral-500">교사 한마디</p>
        {notes.length > 1 && (
...
function EventsList({ events }: { events: NoticeWidgetEvent[] }) {
  return (
    <div className="rounded border border-neutral-100 p-3">
      <p className="text-xs font-semibold text-neutral-500">할일 · 공지</p>
      {events.length === 0 ? (
          ...
          {events.map((e) => (
            <li key={e.id}>
              <div className="flex gap-2">
                ...
                <span className="font-medium text-neutral-700">{e.title}</span>
              </div>
              {e.content && (
