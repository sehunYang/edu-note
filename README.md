...
<sub>실제로 매일 쓰고 있는 화면입니다. 학생 이름·이메일·출결 사유는 흐리게 처리했습니다.</sub>
...
**"진도 맞출 수 있나?"에 숫자로 답합니다.** 시험 날짜만 넣으면 분반마다 남은 차시가 자동으로 계산되고,
이번 주에 해야 할 차시와 밀린 차시가 따로 모입니다.
...
출결은 **누가 · 언제 · 어떤 성격으로 · 몇 교시** 빠졌는지가 한 줄에 정리됩니다.
증빙 서류가 `미제출`인 건은 눈에 띄게 표시되어, 누구를 재촉해야 하는지 한눈에 보입니다.
...
설치 상태·기능 on/off·현재 버전은 앱 안 **세팅실 → 시스템 상태**에서 한눈에 볼 수 있습니다.
나이스 인증키도 여기서 넣습니다 — Vercel 대시보드에 들어갈 일이 없습니다.

![시스템 상태](docs/images/07-system.png)
